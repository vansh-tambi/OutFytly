      ...
      <div className="max-w-7xl mx-auto">
        ...
        {validItems.length === 0 ? (
          <div className="text-center py-20 bg-plum/30 rounded-lg">
            ...
            <Link to="/browse">
              <motion.button whileHover={{ scale: 1.05 }} className="mt-6 bg-primary px-6 py-2 rounded-lg font-semibold">
                Start Shopping
              </motion.button>
            </Link>
          </div>
              ...
              <AnimatePresence>
                {validItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <img 
                        src={imageUrl} // ✅ FIX: Use the full Cloudinary URL directly
                      ...
                      <div className="flex-grow">
                        ...
                        <p className="text-primary font-semibold mt-1">₹{item.product.rentalPrice.toLocaleString()}</p>
                      </div>
                      <div className="flex flex-col items-end justify-between h-24">
                        <div className="flex items-center gap-2 bg-ink px-2 py-1 rounded-md">
                          <button onClick={() => updateQuantity(item._id, item.quantity - 1)} className="text-lavender"><Minus size={16}/></button>
                          <span className="font-bold text-white w-4 text-center">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item._id, item.quantity + 1)} className="text-lavender"><Plus size={16}/></button>
                        </div>
                        <button onClick={() => removeItem(item._id)} className="text-red-400 hover:text-red-500 transition-colors flex items-center gap-1 text-sm">
